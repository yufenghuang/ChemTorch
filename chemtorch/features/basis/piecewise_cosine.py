...
def get_d_features(Rij, Rijk, Rhat, M2, M3, Router=6, Rinner=0):
    ...
    d_phi3 = np.zeros((*Rijk.shape, M3))

    R2 = Router - (Router - Rinner) / M2
    R3 = Router - (Router - Rinner) / M3

    phi2[Rij > 0] = piecewise_cosine(Rij[Rij>0], M2, Rinner=Rinner, Router=R2)
    phi3_ijk[Rijk > 0] = piecewise_cosine(Rijk[Rijk>0], M3, Rinner=Rinner, Router=R3)
    d_phi2[Rij > 0] = diff_p_cosine(Rij[Rij>0], M2, Rinner=Rinner, Router=R2)
    d_phi3[Rij > 0] = diff_p_cosine(Rij[Rij>0], M3, Rinner=Rinner, Router=R3)
    ...
    g3_dldl = g3_dldl.reshape((nAtoms, M3, 3, M3, M3)).transpose([0, 1, 3, 4, 2])
    g3_dldl = g3_dldl + g3_dldl.tranpose([0, 2, 1, 3, 4])
    ...
    g3_dpdl_C = g3_dpdl_C.reshape((nAtoms, M3, 3, maxNb, M3)).transpose([0, 3, 4, 1, 2])
    g3_dpdl_C = g3_dpdl_C[:, :, None, :, :, :] * phi3_ij[:, :, :, None, None, None]

    g3_dpdl = g3_dpdl_A + g3_dpdl_C
    g3_dpdl = g3_dpdl + g3_dpdl.transpose([0, 1, 3, 2, 4, 5])

    g_dldl = np.concatenate([g2_dldl, g3_dldl.reshape((nAtoms, -1, 3))], axis=1)
    g_dpdl = np.concatenate([g2_dpdl, g3_dpdl.reshape((nAtoms, maxNb, -1, 3))], axis=2)
