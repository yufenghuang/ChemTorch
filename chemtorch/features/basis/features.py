...
def get_dG_dR(Rfrac, lattice, M2, M3, Rinner=0.0, Router=6, basis='cosine'):
    lattice = standardize_lattice(lattice)
    Rcart = frac2cart(Rfrac, lattice)
    idxNb, Rij, maxNb = get_nb(Rcart, lattice, dcut=6.2)
    dij, dijk, Rhat = get_distances(Rij)
    G, dGl_dRl, dGp_dRl = p_cosine.get_d_features(dij, dijk, Rhat, M2, M3, Rinner=Rinner, Router=Router)
    if basis == 'cosine':
        pass
    return G, dGl_dRl, dGp_dRl
